...
public class Diff_match_patch {
   ...
   * @return HTML representation.
   */
  public String diff_LineText(LinkedList<Diff> diffs) {
    StringBuilder html = new StringBuilder();
    for (Diff aDiff : diffs) {
      ...
      case DELETE:
    	  ...
    	  if(!(text.replaceAll("<TAB/>", "")).endsWith("\n")) {
  ...
  public ArrayList<String> diff_LineTextList(LinkedList<Diff> diffs) {
	  
		String[] textLineArr = this.diff_LineText(diffs).toString().split("\n", -1);
	    
		ArrayList<String> diffReport = new ArrayList<String>();
		ArrayList<Integer> lines = new ArrayList<Integer>();
	    
		for(int ii=0; ii<textLineArr.length; ii++) {
	    	if(textLineArr[ii].indexOf("<//del>") != -1) {
	    		
	    		if(!(textLineArr[ii].replaceAll("<//del>", "").trim()).equals("")) {
	    			lines.add(ii+1);
	    			diffReport.add(textLineArr[ii].replaceAll("<//del>", "").replaceAll("<//ins>", ""));
	    		}
	    	} else if(textLineArr[ii].indexOf("<//ins>") != -1) {
	    		lines.add(ii+1);
	    		diffReport.add(textLineArr[ii].replaceAll("<//ins>", ""));
	    	}
	    }
		...
		for(int ii=1; ii<lines.size(); ii++) {
			...
			if((line+1)==lines.get(ii)) {
				lineNumbers.add(ii,"");
				isCont = true;
			} else {
				if(isCont) {
					String firstNumber = lineNumbers.get(firstLine);
					lineNumbers.remove(firstLine);
					lineNumbers.add(firstLine, firstNumber+" ~ "+lines.get(ii-1));
					lineNumbers.add(ii,"<Line> " + lines.get(ii));
					firstLine = ii;
				} else {
					lineNumbers.add(ii,"<Line> "+lines.get(ii));
					firstLine = ii;
		...
		ArrayList<String> lastDiffReport = new ArrayList<String>();
	    for(int ii=0; ii<diffReport.size(); ii++) {
	    	String lineNum = lineNumbers.get(ii);
	    	if(!"".equals(lineNum)) {
	    		lastDiffReport.add("\n"+lineNum);
	    	}
	    	
	    	lastDiffReport.add(diffReport.get(ii));
	    }
	    
	    if(isDelete && lastDiffReport.size() == 0)
	    	lastDiffReport.add("");
	    
	    return lastDiffReport;
